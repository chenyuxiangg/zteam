...
> ⚠️ §1-§10 为 **v1 历史章节**（独立全链时代，路径为旧 workspace/ 结构）；**当前状态机（v2 模块中心）见 §11**——项目数据在映射表 work_path（默认 ~/project/<项目>/），全局审计在 zteam/logs/。

## 1. 工作区布局与文件命名约定（v1 历史）
...
| `test` | test-developer | test-reviewer | `tests/` | MiniMax-M3（minimax-cn） |
