...
# 拆分后子模块委托（modules / qa / versions）—— commit 4d 兼容 shim
from statectl_core.modules import (
    MODULE_TYPES,
    cmd_module as cmd_module_impl,
    _module_unblock as _module_unblock_impl,
    release_module as release_module_impl,
    _module_stale_recovery as _module_stale_recovery_impl,
    _version_stale_recovery as _version_stale_recovery_impl,
    _module_inputs as _module_inputs_impl,
    _issue_stale_watch as _issue_stale_watch_impl,
    _dep_cycle as _dep_cycle_impl,
    _se_triage_and_attach as _se_triage_and_attach_impl,
    _schedule_module_iter as _schedule_module_iter_impl,
    release_st_v2 as release_st_v2_impl,
    _module_checks as _module_checks_impl,
    _mechanism_selftest as _mechanism_selftest_impl,
)
from statectl_core.qa import (
    release_qa as release_qa_impl,
    confirm_guide as confirm_guide_impl,
    reject_guide as reject_guide_impl,
    _schedule_st_qa as _schedule_st_qa_impl,
...
def cmd_module(project: str, action: str, rest: list):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    return cmd_module_impl(project, action, rest)
...
def release_module(project: str, module: str, iter_n: str, action: str, product: str, conclusion: str = ""):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    return release_module_impl(project, module, iter_n, action, product, conclusion)
...
def _module_stale_recovery(project: str, md: dict, alarms: list):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    _module_stale_recovery_impl(project, md, alarms)
def _version_stale_recovery(project: str, vd: dict, alarms: list):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    _version_stale_recovery_impl(project, vd, alarms)
def _module_inputs(project: str, m: dict, st: dict):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    return _module_inputs_impl(project, m, st)
def _issue_stale_watch(project: str, alarms: list):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    _issue_stale_watch_impl(project, alarms)
def _dep_cycle(md: dict, module: str):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    return _dep_cycle_impl(md, module)
def _se_triage_and_attach(project: str, vd: dict, md: dict, alarms: list):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    _se_triage_and_attach_impl(project, vd, md, alarms)
def _schedule_module_iter(project: str, vd: dict, md: dict, st: dict, alarms: list):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    _schedule_module_iter_impl(project, vd, md, st, alarms)
def release_st_v2(project: str, version: str, product: str, conclusion: str):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    return release_st_v2_impl(project, version, product, conclusion)
def _module_checks(project: str, module: str):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    return _module_checks_impl(project, module)
def _mechanism_selftest(project: str):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    return _mechanism_selftest_impl(project)
def release_qa(project: str, version: str, product: str, conclusion: str):
    # ↑ 已迁移 statectl_core.qa（commit 4d 兼容 shim）
    return release_qa_impl(project, version, product, conclusion)
def confirm_guide(project: str, version: str):
    # ↑ 已迁移 statectl_core.qa（commit 4d 兼容 shim）
    return confirm_guide_impl(project, version)
def reject_guide(project: str, version: str, reason: str):
    # ↑ 已迁移 statectl_core.qa（commit 4d 兼容 shim）
    return reject_guide_impl(project, version, reason)
def _schedule_st_qa(project: str, vd: dict, md: dict, st: dict, alarms: list):
    # ↑ 已迁移 statectl_core.qa（commit 4d 兼容 shim）
    _schedule_st_qa_impl(project, vd, md, st, alarms)
def release_st_case(project: str, version: str, product: str, conclusion: str):
    # ↑ 已迁移 statectl_core.qa（commit 4d 兼容 shim）
    return release_st_case_impl(project, version, product, conclusion)
...
def _module_unblock(project: str, module: str, iter_n: str):
    # ↑ 已迁移 statectl_core.modules（commit 4d 兼容 shim）
    return _module_unblock_impl(project, module, iter_n)
...
def _sync_project_version(project: str, version: str):
    # ↑ 已迁移 statectl_core.versions（commit 4d 兼容 shim）
    return _sync_project_version_impl(project, version)
