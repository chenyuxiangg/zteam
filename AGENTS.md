...
1. 你的启动指令（`hermes chat -q` 的 query）会指明：角色（八角色：PM/SE/TE/MDE/FO/MTO/STO/QA）、对象（需求 `<project>/<req_id>` 或模块 `<project>/__mde-{模块}-it{N}` 或版本 `<project>/__arch{版本}`）与轮次（`N`）；
...
```
zteam/                     # 资产层（git 跟踪）
├── roles/  docs/  scripts/  skills/     # 角色定义 / 文档 / 代码
...
项目数据不在 zteam 内——每个项目在**映射表 work_path**（默认 ~/project/<项目名>/）：
├── status.json / status.lock    # 该项目状态机与项目锁（项目间并行、同项目串行）
├── input/                       # 该项目需求投放区
...
└── logs/                        # 该项目 worker 日志
你的 cwd = 项目 work_path；产物/状态相对路径即落在 work_path 下
```

## 完成标志（三件套，缺一不可）

1. **产物落盘**：按指令写 `{work_path}/<阶段目录>/{req_id}-r{N}.md`（相对 cwd，你的 cwd 即项目 work_path）（每轮新文件，不覆盖旧版本）；
...
3. **审计日志**：状态命令自动写入 `zteam/logs/pipeline.log`（全局审计）,无需手动追加。

## 失败时

- 若在写产物前决定放弃本轮：运行 `python3 scripts/statectl.py rollback {project}/{req_id}`（自动 `failures + 1` 并回到该阶段可认领状态）；
- 若进程直接崩溃、没来得及处理：保持中间态原样即可——上半部 **stale 恢复**（进程死亡兜底）与**巡检**会自动兜底，无需人工；
- 无论何种失败，把你的 stdout 留在 `{work_path}/logs/worker-{req_id}-r{N}.log` 中供人工排查。

## 禁止

- 不得修改需求原文（`{work_path}/input/` 下的文件）；
