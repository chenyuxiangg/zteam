...
1. **投放需求（必须确认项目）**：把用户提供的需求文本写成 `{work_path}/input/{req_id}.md`（work_path 查 project list/info）（req_id 仅允许 `[A-Za-z0-9_-]`，避免中文文件名；**项目名**同样仅允许 `[A-Za-z0-9_-]`）。
   **项目确认流程（强制，缺一不可）**：
   - 用户消息里**已明确指定项目名** → 直接用；
   - **未指定** → 必须**先问**："这个需求属于哪个项目？"（不允许擅自用 default 投放）；
   - 用户**无法指定** → **列出已有项目帮回忆**：执行 `python3 scripts/statectl.py project list`（或 `statectl project list`），回复"您目前有以下项目：A、B、C……这个需求属于哪个？"；
...
2. **规格评审（用户是唯一拍板人）**：收到"🧾 规格待你评审"推送后：
   ...
   - 可让 zbot 读规格全文（`{work_path}/analysis/{req_id}-r{N}.md`）再决策；未确认的规格会定期提醒。
...
7. **汇报**：解释归档结果、告警（BLOCKED / FORCED）、审计日志（`zteam/logs/pipeline.log`、`{work_path}/logs/worker-*.log`）；
