# 功能模块设计：game-app（snake-linux v2.0.0 迭代 4）r1（首发）
...
> 4. **打包内置字体**（G4-5）：`_load_cjk_font` 走 `sys._MEIPASS`/`__file__` 邻近路径查找内置字体文件（如 `SourceHanSansCN-Regular.otf`），避免 Linux 字体版本差异导致中文字形缺失
...
| 输出打包资源 | `dist/`：`snake-gui`（Linux ELF）+ `snake-gui.exe`（Windows）+ `snake-gui.app`（macOS，含 Intel + Apple Silicon 双架构）+ `SHA256SUMS` + `RELEASE_NOTES.md` + `USER_GUIDE.md` + 内置字体 `SourceHanSansCN-Regular.otf` |
...
- ✅ **发布物 SHA256SUMS**（G4-1，NFR-07）：三平台包校验和
- ✅ **可读错误提示完善**（G4-2，NFR-03）：SDL 驱动缺失 / HiDPI 异常 / CJK 字体全失败 / 平台不兼容（macOS <12 / Windows <10） / 用户数据目录不可写 等可预见失败均输出人类可读 stderr
- ✅ **错误退出码扩展**（G4-2）：0 正常 / 1 app 异常 / 2 图形环境不可用 / 3 用户数据目录不可写（新增，区分错误类型便于用户/脚本判断）
- ✅ **性能 profile 脚本**（G4-3，NFR-01/02）：`scripts/bench_fps.py` + `scripts/bench_memory.py`，跑 60 秒基准，记录 P95 帧时间 / 平均 FPS / 内存峰值留档
- ✅ **用户指南**（G4-4，FR-16）：`snake-linux/release/USER_GUIDE.md`（五节齐全：下载运行 / 键位表 / 难度 / 皮肤 / 暂停 / 平台差异 / 已知限制）
- ✅ **打包内置字体**（G4-5）：`SourceHanSansCN-Regular.otf`（或同源开源中文字体，license 兼容——优先选用 OFL 协议）打包进 `game_app/fonts/` 模块目录；`_load_cjk_font` 优先查内置文件 → 失败再走 `pygame.font.match_font` 回退链
- ✅ **发布说明**（G4-6）：`snake-linux/release/RELEASE_NOTES.md`（v2.0.0 changelog + 三平台下载链接 + 校验和）
- ✅ **回归全模块**（G4-7）：game-app iter-4 测试用例覆盖错误路径 + 打包脚本冒烟测试 + 用户指南字段完备性
...
6. **打包专用目录**：`snake-linux/code/game-app/iter-3/game_app/fonts/` 内置字体文件（PyInstaller `--add-data` 注入）；`snake-linux/scripts/build_*.{sh,bat}` 构建脚本；`snake-linux/spec/snake-gui.spec` PyInstaller 配置。
...
8. **macOS 双架构**：PyInstaller `--target-arch universal2` 或分别在 Intel / Apple Silicon 构建机上分别构建；产物命名 `snake-gui-intel.app` / `snake-gui-arm64.app`（或合并为 `snake-gui.app` + `lipo` 合成）。
9. **PyInstaller 版本**：≥5.13（支持 Python 3.10+ 打包；>=5.0 已稳定支持 `--onefile --windowed`）。
10. **退出码语义**（G4-2 新增退出码 3）：
    - 0 = 正常退出（用户主动退出 / 游戏结束自然退出）
    - 1 = app 异常（ConfigError / StorageError / 未捕获异常 / 数据损坏）
    - 2 = 图形环境不可用（Renderer.init 失败 / SDL 缺失 / pygame.error）
    - 3 = 用户数据目录不可写（HighScoreStore mkdir 失败 / disk full / 权限拒绝）
...
### 1.3 路径定位常量（**G4-1/G4-5 新增**）

```python
# _constants.py — iter-3 沿用 + iter-4 扩展
...
def get_bundled_font_path() -> str:
    ...
    源码模式 / --onedir 模式下文件位于 __file__ 同目录；
...
| **INV-17** | **G4-2 新增**：GraphicsUnavailableError 必映射退出码 2，StorageUnavailableError 必映射退出码 3 |
...
| **INV-20** | **G4-5 新增**：打包内置字体优先级 = `sys._MEIPASS` 内文件 > `__file__` 邻近文件 > `pygame.font.match_font` 回退链 > `pygame.font.Font(None, size)` |
...
| **app → 字体文件**（**G4-5 新增**） | `pygame.font.Font(get_bundled_font_path(), size)` 或 `Font(None, size)` 兜底 | 路径定位走 `sys._MEIPASS` 优先 |
...
- **打包资源**：字体文件 `SourceHanSansCN-Regular.otf` 通过 PyInstaller `--add-data` 注入（spec 文件声明）。
- **构建脚本状态**：构建脚本输出到 `dist/`（gitignored）；SHA256SUMS / RELEASE_NOTES / USER_GUIDE 跟踪到 git。
...
class App:
    ...
    - CJK 字体打包内置文件优先（G4-5）：get_bundled_font_path() 走 sys._MEIPASS → __file__ → match_font 兜底
    - 性能 profile 集成（G4-3）：_on_bench_complete 钩子供 scripts/bench_fps.py 调用
    ...
    def _load_cjk_font(self, size: int, bold: bool = False) -> pygame.font.Font:
        """G4-5 修订：优先打包内置字体 → match_font 回退链 → Font(None) 兜底。
...
| **`get_bundled_font_path()`** | **function `_constants.py`** | **iter-4 G4-5 新增**：定位打包内置字体 |
| **`scripts/build_*.{sh,bat}`** | **构建脚本** | **iter-4 G4-1 新增**：三平台 PyInstaller 构建 |
| **`spec/snake-gui.spec`** | **PyInstaller spec** | **iter-4 G4-1 新增**：打包配置（entry / datas / hiddenimports） |
| **`scripts/bench_fps.py`** | **性能脚本** | **iter-4 G4-3 新增**：NFR-01 帧率实测 |
| **`scripts/bench_memory.py`** | **性能脚本** | **iter-4 G4-3 新增**：NFR-02 内存实测 |
| **`snake-linux/release/USER_GUIDE.md`** | **文档** | **iter-4 G4-4 新增**：五节齐全用户指南 |
| **`snake-linux/release/RELEASE_NOTES.md`** | **文档** | **iter-4 G4-6 新增**：v2.0.0 changelog |
| **`snake-linux/release/SHA256SUMS`** | **文件** | **iter-4 G4-1 新增**：三平台包校验和 |
...
│       ├── errors.py                            # iter-4 G4-2 扩展：退出码 3 + 警告类
│       ├── perf.py                              # iter-4 G4-3 新增：性能常量
│       ├── _constants.py                        # iter-4 G4-5 新增：get_bundled_font_path
│       └── fonts/                                # iter-4 G4-5 新增：打包内置字体目录
│           └── SourceHanSansCN-Regular.otf       # OFL 协议开源中文字体
├── scripts/                                     # iter-4 G4-1/G4-3 新增
│   ├── build_linux.sh                           # Linux ELF 构建脚本
│   ├── build_windows.bat                        # Windows .exe 构建脚本
│   ├── build_macos.sh                           # macOS .app 构建脚本
│   ├── bench_fps.py                             # G4-3 NFR-01 帧率实测
│   ├── bench_memory.py                          # G4-3 NFR-02 内存实测
│   └── gen_sha256sums.sh                        # G4-1 三平台包校验和生成
├── spec/                                        # iter-4 G4-1 新增
│   └── snake-gui.spec                           # PyInstaller 配置文件
└── release/                                     # iter-4 G4-1/G4-4/G4-6 新增
    ├── USER_GUIDE.md                            # G4-4 用户指南
    ├── RELEASE_NOTES.md                         # G4-6 发布说明
    └── SHA256SUMS                                # G4-1 校验和
```

### 4.2 主循环骨架（沿用 iter-3 + G4-2 错误捕获扩展）
...
### 4.7 初始化（**G4-2 HiDPI 降级 + G4-5 字体路径 + G4-2 StorageUnavailableError 退出码 3**）
...
def _load_cjk_font(size: int, bold: bool = False) -> pygame.font.Font:
    ...
    iter-4 G4-5 优先级（INV-20）：
    1. 打包内置字体文件（SourceHanSansCN-Regular.otf）—— PyInstaller --onefile 临时目录 / 源码目录
...
### 4.9 PyInstaller spec 文件（**G4-1 新增**）
...
# 生成 SHA256SUMS
sha256sum dist/snake-gui-linux-x86_64 > dist/SHA256SUMS
...
REM 生成 SHA256SUMS（使用 certutil）
certutil -hashfile dist\snake-gui-windows-x86_64.exe SHA256 > dist\SHA256SUMS
...
if command -v lipo &> /dev/null; then
    ...
    mkdir -p dist/snake-gui.app/Contents/MacOS
    lipo -create \
...
# 生成 SHA256SUMS
shasum -a 256 dist/snake-gui.app/* > dist/SHA256SUMS
...
# 用途：发布前统一生成校验和
# 前置：dist/snake-gui{,-linux,-windows,-macos} 已生成
...
from perf import MEMORY_PEAK_MB_MAX, BENCH_DURATION_SECONDS
...
### 4.12 用户指南 USER_GUIDE.md（**G4-4 新增**，FR-16）
...
### 4.13 发布说明 RELEASE_NOTES.md（**G4-6 新增**）
...
- **iter-4 增量入口**仅 4 处：错误类型扩展（`errors.py` G4-2）+ HiDPI 降级包装（`_init_pygame` G4-2）+ 字体加载路径（`_load_cjk_font` G4-5）+ 性能脚本（`scripts/` G4-3），**最小侵入**。

### 5.2 可扩展性（Extensibility）

- **AppConfigV4 子类化扩展**：iter-4+ 若加新运行期常量，可继续子类化 `AppConfigV4(AppConfigV3)`，**不破坏 iter-1/2/3 既有 `AppConfig` 兼容性**。
- **错误类型扩展**：新增异常类只需在 `_EXIT_CODE_MAP` 添加映射即可，**无需修改 main() 退出逻辑**。
- **打包资源扩展**：新增字体/图片等静态资源只需在 `spec/snake-gui.spec` 的 `datas` 列表添加条目即可，**app 代码无需修改**。
- **性能脚本扩展**：新增基准维度只需在 `perf.py` 添加常量 + 新建 `scripts/bench_*.py`，**主程序无需修改**。

### 5.3 可部署性（Deployability）

- **PyInstaller 单文件**（G4-1）：`--onefile --windowed --name snake-gui --collect-submodules game_app --collect-submodules platform_storage --collect-submodules gui_renderer --add-data "fonts/SourceHanSansCN-Regular.otf:fonts"`
- **三平台原生构建**（G4-1）：Linux 在 Linux 构建机 / Windows 在 Windows 构建机 / macOS 在 macOS 构建机（PyInstaller 不支持交叉打包）
- **macOS 双架构**（G4-1）：`--target-arch x86_64` + `--target-arch arm64` + `lipo` 合并为 `universal2`
...
- **性能脚本可测**：mock `time.perf_counter()` + `resource.getrusage()` → 断言 PASS/FAIL 判定逻辑正确。
- **用户指南字段完备性**：解析 `USER_GUIDE.md` Markdown，断言五节齐全（下载运行/键位表/难度/皮肤/暂停/平台差异/已知限制）。
- **构建脚本冒烟**：UT 用 `subprocess.run(['bash', 'scripts/build_linux.sh'], ...)` 在临时目录运行，断言生成 `dist/snake-gui-linux-x86_64` 文件（**仅 Linux CI 环境可测**）。

### 5.5 鲁棒性 / 韧性（**G4-2 新增 + iter-1/2/3 沿用**）
...
| **用户数据目录不可写**（G4-2） | `HighScoreStore.mkdir/save` 抛 OSError → 抛 StorageUnavailableError → **退出码 3**（iter-4 新增，区分于一般 AppError）+ stderr suggestion（"请检查 ~/.local/share 目录权限"） |
| **CJK 字体回退**（G4-5） | 打包内置字体 + match_font 回退链全失败 → SDL 默认字体 + stderr warning（CJKFontFallbackWarning） + `_cjk_font_fallback=True`（INV-19）→ **不退出**，中文显示为方框 |
| **平台版本不兼容**（G4-2） | macOS <12 / Windows <10 → stderr warning（PlatformUnsupportedWarning）→ **不退出**，尽力兼容（INV-19） |
...
### 5.6 错误处理矩阵（iter-4 G4-2 全量修订 + iter-3 G3-2 沿用）
...
| **HighScoreStore.save 失败** | **3** | **同上** |
| `_dispatch_menu(RESET_HIGHSCORE)` 失败 | 3 | 同上 |
| `score_callback` 内 `storage.save` 失败 | 3 | 同上 |
...
└── test_app_iter4.py                  # G4-2/3/4/5 增量 UT
    ├── test_errors.py                 # G4-2：错误类型 + 退出码映射
    ├── test_hidpi_fallback.py         # G4-2：HiDPI 降级
    ├── test_platform_check.py         # G4-2：平台版本检查
    ├── test_font_loading.py           # G4-5：内置字体优先 + 回退链
    ├── test_perf_constants.py         # G4-3：性能常量正确性
    ├── test_bench_fps.py              # G4-3：NFR-01 帧率基准脚本
    ├── test_bench_memory.py           # G4-3：NFR-02 内存基准脚本
    ├── test_user_guide.py             # G4-4：用户指南字段完备性
    ├── test_release_notes.py          # G4-6：发布说明字段完备性
    └── test_sha256sums.py             # G4-1：SHA256SUMS 生成正确性
```
...
| **ERR-5** | test_error_to_exit_code_mapping | error_to_exit_code(GraphicsUnavailableError) == 2；error_to_exit_code(StorageUnavailableError) == 3；error_to_exit_code(ConfigError) == 1 | G4-2 |
...
| **PLAT-2** | test_windows_old_version_warning | Windows <10 → PlatformUnsupportedWarning | G4-2 |
| **PLAT-3** | test_unsupported_platform_continues | 平台警告后 run() 仍继续（不退出） | G4-2 |
| **FONT-1** | test_bundled_font_priority | get_bundled_font_path 返有效路径 → 优先用内置字体 | G4-5 |
...
| **SHA-1** | test_sha256sums_format | SHA256SUMS 文件每行格式 = "<64-hex>  <file>" | G4-1 |
| **SHA-2** | test_sha256sums_match_actual | 重新计算 SHA256 与文件中记录匹配 | G4-1 |
| **BUILD-1** | test_build_linux_smoke（**仅 Linux CI 环境**） | bash scripts/build_linux.sh → 生成 dist/snake-gui-linux-x86_64 | G4-1 |
| **BUILD-2** | test_build_windows_smoke（**仅 Windows CI 环境**） | scripts/build_windows.bat → 生成 dist/snake-gui-windows-x86_64.exe | G4-1 |
| **BUILD-3** | test_build_macos_smoke（**仅 macOS CI 环境**） | bash scripts/build_macos.sh → 生成 dist/snake-gui.app | G4-1 |
| **SPEC-1** | test_spec_file_syntax | spec/snake-gui.spec 语法正确（PyInstaller --clean --dry-run） | G4-1 |
| **SPEC-2** | test_spec_datas_include_fonts | datas 列表包含 SourceHanSansCN-Regular.otf | G4-1 |
...
**第一阶段（G4-2 错误处理 + 退出码 3）**：
1. 写 `test_errors.py`（UT ERR-1/2/3/4/5）→ 跑（红）→ 改 `errors.py` 加 StorageUnavailableError + suggestion 字段 + error_to_exit_code 函数（绿）
...
4. 写 `test_platform_check.py`（UT PLAT-1/2/3）→ 跑（红）→ 写 `_check_platform_version`（绿）
5. 改 `app.py.run` 加 StorageUnavailableError → exit 3 + GraphicsUnavailableError suggestion 写入 stderr（§4.2）

**第二阶段（G4-5 字体加载）**：
6. 下载 `SourceHanSansCN-Regular.otf`（OFL 协议）放到 `game_app/fonts/` 目录
7. 写 `_constants.py.get_bundled_font_path()`（§1.3）→ 跑（红）→ 写（绿）
8. 改 `fonts.py._load_cjk_font` 优先内置字体（§4.8）→ 跑（红）→ 改（绿）
9. 写 `test_font_loading.py`（UT FONT-1/2/3/4/5）→ 跑

**第三阶段（G4-3 性能脚本）**：
10. 写 `perf.py` 性能常量（§1.2）→ 跑（红）→ 写（绿）
11. 写 `scripts/bench_fps.py`（§4.11.1）→ 跑（红）→ 写（绿）
12. 写 `scripts/bench_memory.py`（§4.11.2）→ 跑（红）→ 写（绿）
13. 写 `test_perf_constants.py` + `test_bench_fps.py` + `test_bench_memory.py`（UT PERF-1~5）→ 跑

**第四阶段（G4-1 打包）**：
14. 写 `spec/snake-gui.spec`（§4.9）→ 跑（红）→ 写（绿）
15. 写 `scripts/build_linux.sh` + `scripts/build_windows.bat` + `scripts/build_macos.sh`（§4.10.1/2/3）→ 跑（红）→ 写（绿）
16. 写 `scripts/gen_sha256sums.sh`（§4.10.4）→ 跑（红）→ 写（绿）
17. 在 Linux 构建机上跑 `bash scripts/build_linux.sh`（**手工**，CI 环境）→ 验证生成 `dist/snake-gui-linux-x86_64`
18. 写 `test_spec_file_syntax.py` + `test_sha256sums.py`（UT SPEC-1/2/3 + SHA-1/2）→ 跑

**第五阶段（G4-4/G4-6 文档）**：
19. 写 `USER_GUIDE.md`（§4.12）→ 写 `test_user_guide.py`（UT GUIDE-1/2/3）→ 跑（红）→ 补字段（绿）
20. 写 `RELEASE_NOTES.md`（§4.13）→ 写 `test_release_notes.py`（UT REL-1/2）→ 跑（红）→ 补字段（绿）
...
24. **手工真实构建验证清单**（FO 实施后三平台构建机）：
    - [ ] Linux 构建机：bash scripts/build_linux.sh → dist/snake-gui-linux-x86_64 → chmod +x → ./snake-gui-linux-x86_64 → 游戏可启动
    - [ ] Windows 构建机：scripts\build_windows.bat → dist\snake-gui-windows-x86_64.exe → 双击运行 → 游戏可启动
    - [ ] macOS 构建机：bash scripts/build_macos.sh → dist/snake-gui.app → 双击运行 → 游戏可启动（Intel + Apple Silicon 各验证一次）
...
- 错误类型扩展原则：新增异常类 → 在 `_EXIT_CODE_MAP` 添加映射 → 不修改 main() 退出逻辑
- 打包资源扩展原则：新增字体/图片 → 在 `spec/snake-gui.spec` 的 `datas` 列表添加条目 → 不修改 app 代码
...
| 项 | v1 终端版 | v2 iter-1/2/3 | **v2 iter-4（G4 增量）** |
|----|----------|---------------|--------------------------|
| 错误提示 | curses 错误退出 + 退出码 | 退出码 0/1/2 | **退出码 0/1/2/3（INV-17）+ stderr suggestion + 非致命警告类** |
| 退出码 | 0/1 | 0/1/2 | **0/1/2/3（G4-2 新增退出码 3）** |
| 字体 | 终端字符 | CJK 字体回退链 | **打包内置字体优先 + 系统字体回退链（G4-5）** |
| 打包 | 源码运行 | 源码运行 | **三平台 PyInstaller 单文件可执行（G4-1）** |
| 性能 | 无实测 | 无实测 | **NFR-01 ≥60FPS / NFR-02 ≤300MB 实测脚本（G4-3）** |
| 用户指南 | README 四节 | README 四节 | **USER_GUIDE.md 五节齐全（G4-4）** |
| 发布物 | snake.py | 源码 | **snake-gui ELF/.exe/.app + SHA256SUMS + RELEASE_NOTES + USER_GUIDE（G4-1/4/6）** |
| HiDPI | 不适用 | AppConfigV3 + enable_high_dpi | **HiDPI 失败自动降级（INV-18，G4-2）** |
| 平台检查 | 无 | 无 | **平台版本检查（macOS <12 / Windows <10）+ 非致命警告（G4-2）** |

---

## 附录 D：SE 评审预留（P0/P1/P2 修订对照）

> iter-4 r1 SE 评审结果将在 `snake-linux/review/design/game-app/iter-4/snake-linux-game-app-design-iter4-r1.md` 记录。
> 本版 r1 为首发版，FO 落地后提交 SE 评审。
...
> - [x] **G4-2**：错误提示完善（退出码 3 + suggestion 字段 + 3 类非致命警告 + HiDPI 降级）
> - [x] **G4-3**：性能 profile 脚本（perf.py 常量 + bench_fps.py + bench_memory.py）
> - [x] **G4-4**：USER_GUIDE.md 五节齐全（下载运行/键位表/难度/皮肤/暂停/平台差异/已知限制）
> - [x] **G4-5**：打包内置字体（SourceHanSansCN-Regular.otf + get_bundled_font_path 路径定位）
> - [x] **G4-6**：RELEASE_NOTES.md（v2.0.0 changelog）
...
> - [x] 跨迭代复用：主循环 ≤ 30 行 / 单一职责 / 4 处增量入口（G4-2 错误处理 + G4-5 字体加载 + 性能脚本 + 文档），最小侵入