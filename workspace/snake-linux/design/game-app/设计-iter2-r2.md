# 功能模块设计：game-app（snake-linux v2.0.0 迭代 2）r2（修订：回应 SE r1 评审 FAIL 意见）

> MDE r2 · 跨迭代复用基线：迭代 1 设计 `snake-linux/design/game-app/设计-r3.md`（SE 评审 PASS）+ 迭代 1 实际代码 `snake-linux/code/game-app/iter-1/`（it_passed）
...
> **本轮修订**：回应 SE 评审 `snake-linux/review/design/game-app/iter-2/snake-linux-game-app-design-iter2-r1.md`（FAIL，2026-08-14）—— 详见 §0.0 修订摘要。本版 r2 全文一致性已自检（代码示例 / UT / 不变量三方对齐）。
...
## 0.0 本轮修订摘要（回应 SE r1 评审 FAIL，2026-08-14）

SE 评审 r1 在 `snake-linux/review/design/game-app/iter-2/snake-linux-game-app-design-iter2-r1.md` 给出 FAIL：2×P0 内部自相矛盾（代码示例与 UT/不变量声明直接冲突）+ 3×P1（异常类型契约错配 / 键位语义破坏 / fixture 注入失效）+ 7×P2 文档/UT 级问题。**r2 全部消化，逐条落地（与 r1 标注文本一致，但实际代码块已重写对齐）**：

| 评审意见 | r2 修订 | 验证位置 |
|----------|---------|----------|
| **P0-1** PAUSED 屏态同步断链 | `_dispatch_playing(TOGGLE_PAUSE/UNFOCUS)` 调 `toggle_pause()` 后**显式** `self.screen = AppScreen.PAUSED`；`_dispatch_paused(TOGGLE_PAUSE)` 调 `toggle_pause()` 后**显式** `self.screen = AppScreen.PLAYING`；`_tick` 内删除 `elif new_status == PAUSED` 死代码（core step 永不返 PAUSED） | §3.4 / §4.4 / §4.5 / §1.4 INV-10/11 / §6.4 UT P-1/P-2/P-3/P-8 |
| **P0-2** INV-13「回调同步 _high_score」与 §4.6 实现矛盾 | `_new_game` 回调**直接写** `self._high_score = max(self._high_score, s)`，删除 nonlocal `_high` / `_high_ref` dict 容器；§3.4 与 §4.6 合并为一处权威实现（以 §4.6 为基底） | §3.4 / §4.6 / §1.4 INV-13 / §6.4 UT SC-1/SC-2 |
| **P1-1** mkdir 失败抛 OSError 而非 StorageError | `_init_pygame` 改 `except (StorageError, OSError) as e: raise AppError("用户数据目录不可写") from e`（同时记 platform-storage issue：__init__ mkdir 失败应包 StorageError） | §4.7 / §5.5 / §6.4 UT S-3 |
| **P1-2** GAME_OVER 态 Q 键被误转 BACK_TO_MENU | `_map_event` ESC 返**独立 action ESCAPE**（与 Q 的 QUIT 区分）；`_drain_events` GAME_OVER 态 ESCAPE → BACK_TO_MENU；Q 始终 QUIT 直通；新增 UT B-1/B-4 区分断言 | §1.1 / §3.3 / §4.3 / §4.4 / §6.4 UT B-1/B-4 |
| **P1-3** fixture 注入 fake_storage 被覆盖 | `_init_pygame` 改 `if self._storage is None: self._storage = create_storage()`（已注入则跳过）；fixture 在 `_init_pygame()` **之后**注入 fake | §4.7 / §6.2 |
| **P2-1** input.py 示例破坏 iter-1 `_pygame_attr` 模式 | §4.3 沿用 iter-1 `_pygame_attr(name)` 延迟读取；fake_pygame fixture 补 `input` 模块替换 | §4.3 / §6.2 |
| **P2-2** UT P-5 引用不存在的 `app_in_game_over` | 新增 `app_in_game_over` fixture（§6.2）；UT P-5 改 `pytest.raises(InvalidStateError)` 调 `app.game_state.toggle_pause()` | §6.2 / §6.4 UT P-5 |
| **P2-3** UT P-6 序列描述含糊 | 补前置步骤（RUN 态 set_direction → P → P） | §6.4 UT P-6 |
| **P2-4** main() 捕获 StorageError 多余 | §3.5 公开 API 表删 StorageError 项；§3.4 / §4.2 与 iter-1 main() 对齐 | §3.5 |
| **P2-5** §3.4 vs §4.6 _new_game 双版本不一致 | 随 P0-2 合并为一处权威实现（以 §4.6 为基底），§3.4 删 _new_game 代码（仅在 docstring 引用 §4.6） | §3.4 / §4.6 |
| **P2-6** draw_pause_overlay 废代码 + 色值硬编码 | §4.8 删 `big_font = ... if False else body_font`；统一 body_font（22px）渲染；色值走 `PAUSE_OVERLAY_ALPHA` 常量 | §4.8 |
| **P2-7** assert 风格与 iter-1 不一致 | `# type: ignore[union-attr]` 注释统一替换 `assert game_state is not None`（iter-1 风格） | §4.4 |

**一致性自检结果**（r2 提交前 MDE 自查）：

- §3.4 docstring 描述 ↔ §4.4 实际代码（_dispatch_*/_tick）：一致（屏态同步方案 A）
- §1.4 不变量 INV-1/10/11 ↔ §4.4/§4.5 代码：不变量由方案 A 单点保证
- §3.4 _new_game docstring ↔ §4.6 _new_game 代码：合并为一处（§4.6 为权威）
- §6.4 UT 断言 ↔ §4.4/§4.6 实现：UT P-1/P-2/P-3/P-8 / SC-1/SC-2 与代码语义一致
- §4.7 _init_pygame ↔ §6.2 fixture：注入顺序 + `if None` 守卫配套
- §4.3 _map_event ↔ §4.4 _drain_events GAME_OVER 覆盖：ESC/Q 区分语义明确
...
| **G2-7** | 应实现 | **"返回菜单"路径**：GAME_OVER 态新增 `InputAction.BACK_TO_MENU`（**P1-2 iter-2 新增 Backspace 键 → BACK_TO_MENU**）→ `self.screen = AppScreen.MENU; self.game_state = None`（重置 INV-7 保护）；`_dispatch_over` 加该分支；MENU 态进 PLAYING 重置所有 game_state 相关字段 | §1.1 §1.3 §3.4 §4.4 §6.4 |
...
| `InputAction`（Enum） | `QUIT` / `START` / `MOVE_UP/DOWN/LEFT/RIGHT` / `TOGGLE_PAUSE` / `RESTART` / `SELECT_EASY/MEDIUM/HARD` / **`RESET_HIGHSCORE`** / **`BACK_TO_MENU`** / **`ESCAPE`**（**P1-2 iter-2 新增**：ESC 独立 action，与 Q 区分）/ **`UNFOCUS`**（内部信号） | iter-2 新增 4 个；`UNFOCUS` 不来自 `_map_event`，仅主循环内检测到失焦时入 actions |
...
class App:
    ...
    def _dispatch_over(self, action: InputAction) -> None:
        ...
        - RESTART → _new_game
        - BACK_TO_MENU（GAME_OVER 态 Backspace → _map_event 直转 或 ESCAPE → _drain_events 覆盖）→ MENU
        - QUIT（来自 pygame.QUIT 窗口关闭或 Q 键，未被 GAME_OVER 覆盖）→ 主循环 break
        """
    ...
    def _new_game(self, difficulty: Difficulty) -> None:
        ...
        **P0-2 权威实现**：实现位于 §4.6（§3.4 与 §4.6 合并一处；P2-5），本节 docstring 仅
        描述调用流程，不重复实现代码。FO 直接按 §4.6 实现即可。
...
_PAUSE_KEY = "K_p"
_RESTART_KEY = "K_r"
# **P1-2 修订**：ESC 在 _map_event 层返回独立 action ESCAPE（与 Q 的 QUIT 区分），
# 由 _drain_events GAME_OVER 态覆盖 ESCAPE → BACK_TO_MENU；Q 始终返 QUIT 直通主循环 break。
_QUIT_KEY = "K_q"
_ESCAPE_KEY = "K_ESCAPE"
_RESET_HIGHSCORE_KEY = "K_h"                                # G2-3 iter-2 新增
_BACK_TO_MENU_KEY = "K_BACKSPACE"                           # G2-7 iter-2 新增（Backspace 直返 BACK_TO_MENU）

# R3-1 沿用：MENU 态保留键（G2-7 扩展加 BACK_TO_MENU 不在 MENU 保留）
# **P1-2 修订**：ESC 在 _map_event 层返 ESCAPE（独立 action，与 Q 区分）；ESCAPE **不在**
# `_MENU_RESERVED_ACTIONS` 内 → MENU 态下 ESCAPE 由 `_drain_events` 兜底转 START（与 iter-1
# "ESC 视为开始"语义一致）；GAME_OVER 态下 ESCAPE 由屏态覆盖转 BACK_TO_MENU。
...
# GAME_OVER 态保留键（G2-7 新增）：QUIT / RESTART / BACK_TO_MENU / ESCAPE（**P1-2** 修订后：ESCAPE 在 GAME_OVER 走屏态覆盖）
_GAME_OVER_RESERVED_ACTIONS = frozenset({
    InputAction.QUIT,
    InputAction.RESTART,
    InputAction.BACK_TO_MENU,
    InputAction.ESCAPE,                                             # P1-2：GAME_OVER 态 ESC 转 BACK_TO_MENU，Q 仍 QUIT
})
...
def _map_event(event: pygame.event.Event) -> Optional[InputAction]:
    ...
    **P1-2 修订**：ESC 在 `_map_event` 层返回**独立 action ESCAPE**（与 Q 的 QUIT 区分），
    Q 始终 QUIT 直通主循环 break；ESC 由 `_drain_events` GAME_OVER 态覆盖为 BACK_TO_MENU。
    ...
    if k == K_q:                                                # P1-2：Q 键 → QUIT（主循环 break）
        return InputAction.QUIT
    if k == K_ESCAPE:                                           # P1-2：ESC 键 → ESCAPE（GAME_OVER 由 _drain_events 覆盖为 BACK_TO_MENU）
...
def _drain_events(self) -> List[InputAction]:
    ...
    **P1-2 修订**：screen==GAME_OVER 时 ESCAPE action → BACK_TO_MENU（仅 ESC 转，Q 保持 QUIT 直通 break）。
    """
    ...
    for ev in raw:
        ...
        if self.screen == AppScreen.MENU:
            ...
            if action == InputAction.ESCAPE:                           # P1-2：ESC 转 BACK_TO_MENU（不影响 Q）
...
def _dispatch_playing(self, action: InputAction) -> None:
    ...
    **P0-1 屏态同步方案 A**：TOGGLE_PAUSE / UNFOCUS 调 `toggle_pause()` 后**显式**写
    `self.screen = AppScreen.PAUSED`（与 §3.4 / §1.4 INV-11 一致；§6.4 UT P-1 验证）。
    """
    if action == InputAction.MOVE_UP:
        ...
        if self.game_state.status == GameStatus.RUN:                   # 仅 RUN 态失焦 → 暂停
...
def _dispatch_paused(self, action: InputAction) -> None:              # G2-1 新增
    ...
    **P0-1 屏态同步方案 A**：TOGGLE_PAUSE 调 `toggle_pause()` 后**显式**写
    `self.screen = AppScreen.PLAYING`（与 §3.4 / §1.4 INV-11 一致；§6.4 UT P-2 验证）。
    """
...
```python
def _tick(self, dt_ms: int) -> None:
    """PLAYING 态累加节拍。G2-1 PAUSED 不进入此函数（主循环判断，见 §4.2）。

    循环内逐拍重读 tick_ms（R3-8）；OVER 自动转 GAME_OVER（G2-1）；
    **P0-1 屏态同步方案 A 修订**：`_tick` 内**不**做屏态切换——
    - PAUSED 切屏发生在 `_dispatch_playing(TOGGLE_PAUSE/UNFOCUS)` / `_dispatch_paused(TOGGLE_PAUSE)`
      调 `toggle_pause()` 后显式 `self.screen = ...`（§4.4 / INV-11）；
    - core `step()` 在 `status != RUN` 时抛 `InvalidStateError`，**永不返回 PAUSED**，
      因此本函数不可能看到 `new_status == PAUSED`，原 `elif` 分支已删除（**死代码**）。
    主循环 INV-10：`screen == PLAYING` 时才调 `_tick`（PAUSED/OVER/MENU/GAME_OVER 均不进入）。
    """
    ...
    while True:
        ...
        if new_status == GameStatus.OVER:
            self.screen = AppScreen.GAME_OVER                          # OVER 自动转 GAME_OVER
            break
        # P0-1 修订：原 `elif new_status == GameStatus.PAUSED` 分支已删除（死代码）：
        # core step() 永不返回 PAUSED（status != RUN 时抛 InvalidStateError），
        # PAUSED↔PLAYING 切屏由 _dispatch_* 内显式赋值（INV-11 方案 A，§4.4）
```

### 4.6 最高分接入（G2-3 score_callback 注册；**P0-2 权威实现**）

> **§3.4 与 §4.6 合并为一处权威实现**（P0-2 / P2-5 修订）：§3.4 仅描述 _new_game 在
> 主循环中的调用流程与 docstring 引用，本节为唯一代码实现位置。FO 仅需按 §4.6 实现，
> 无需参考 §3.4 重复实现。
    ...
    **G2-3 + P0-2 权威实现**（仅本节，§3.4 不重复）：
    ...
    2. 若 `_storage is not None`（生产环境）：注册回调 `lambda s: self._on_score(s)`，
       ...
       - `self._high_score = max(self._high_score, s)`  ← INV-13 同步实例字段
         （HUD / draw_game_over 每帧读 self._high_score 立即可见）
    ...
    if self._storage is None:
        ...
        def cb(score: int) -> None:
            try:
                self._high_score = max(self._high_score, score)        # INV-13 P0-2 直接写实例字段
...
13. **score_callback 闭包陷阱（**P0-2 修订**）**：r2 回调内**直接** `self._high_score = max(self._high_score, score)`（写实例字段，无需 nonlocal/容器）；UT 用 fake storage 时同样可工作（fake_storage.save 副作用可验证；self 实例字段天然可断言）。
...
| # | 场景 | 断言 |
|---|------|------|
| **B-1**（**P1-2 修订**） | GAME_OVER 态 ESC → BACK_TO_MENU | `app_in_game_over` + fake_pygame 注入 `event = Event(KEYDOWN, key=K_ESCAPE)` → `_map_event` 返 `InputAction.ESCAPE`（P1-2 独立 action）→ `_drain_events` GAME_OVER 态 ESCAPE → BACK_TO_MENU（屏态覆盖）→ `_dispatch_over(BACK_TO_MENU)` → `screen == MENU` + `game_state is None`（INV-7）；**不**含 QUIT |
...
| **B-3** | GAME_OVER 态 pygame.QUIT 仍为 QUIT（不被覆盖） | `pygame.event.Event(pygame.QUIT)` → `_map_event` 返 QUIT（event.type==QUIT 守卫）→ `_drain_events` 在 GAME_OVER 态**不**覆盖（P1-2 修订后仅 ESCAPE 被覆盖；window QUIT 仍 QUIT）→ actions 含 `[QUIT]` 不含 BACK_TO_MENU |
| **B-4**（**P1-2 修订**） | GAME_OVER 态 Q 键 → QUIT 直通 | `app_in_game_over` + `event = Event(KEYDOWN, key=K_q)` → `_map_event` 返 QUIT（K_q 直通）→ `_drain_events` GAME_OVER 态**不**覆盖（P1-2 修订后仅 ESCAPE 被覆盖，Q 仍 QUIT）→ actions 含 `[QUIT]` 不含 BACK_TO_MENU；主循环 break 退出 |
| B-5 | MENU 态 ESC 转 START | `app`（MENU 态）+ fake_pygame 注入 KEYDOWN K_ESCAPE → `_map_event` 返 `InputAction.ESCAPE`（P1-2）→ `_drain_events` MENU 态 ESCAPE 不在 `_MENU_RESERVED_ACTIONS` 中（P1-2 修订：ESCAPE **不在**保留键集合）→ 兜底转 START |
