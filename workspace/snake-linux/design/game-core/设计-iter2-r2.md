...
> MDE r2（修订版，回应 SE 评审 FAIL）· 跨迭代复用基线：迭代 1 设计 `snake-linux/design/game-core/设计-r2.md`（SE 评审 PASS）
> 依据：架构设计 `snake-linux/arch/v2.0.0/架构设计.md`（§迭代计划迭代 2）+ 功能模块分工表 + 需求规格 `snake-linux/analysis/snake-gui-r1.md`（approved）
> r1 评审：`snake-linux/review/design/game-core/iter-2/snake-linux-game-core-design-iter2-r1.md`（FAIL）
> r2 修订要点（响应 SE 评审 P1-1 / P2-A/B/C / P3-A/C/D/E/F/G）：见下表 + 附录 C
> 迭代 2 范围（增量三件套）：
...
| C2-8 | 约束 | **速度曲线保证 NFR-01 量化**：score=0 时三档节拍 = 250/160/100；任何 score 下 `tick_ms(HARD, score) <= tick_ms(EASY, score) * 0.5`；**r2 修订：三档独立下限（EASY_MIN=100 / MEDIUM_MIN=80 / HARD_MIN=50），避免 score 极大时 1:1 持平破坏 50% 约束** | §1.2 / §5.4 #22~25 |
...
# 加速曲线：score 越大、tick_ms 越小（蛇越快）；三档曲线**下限独立**（满足 NFR-01 50% 约束）
# 公式：tick_ms = max(MIN_TICK_MS[difficulty], base - k * score)
#   - EASY:   base=250, k=4   → score=0..37 时 250→下限 100；score≥38 钳制 EASY_MIN=100
#   - MEDIUM: base=160, k=4   → score=0..20 时 160→下限 80；score≥21 钳制 MEDIUM_MIN=80
#   - HARD:   base=100, k=3   → score=0..16 时 100→下限 50；score≥17 钳制 HARD_MIN=50
#   - **三档下限独立**（EASY=100 / MEDIUM=80 / HARD=50），避免共用 50ms 下限导致 score≥40 时 EASY 与 HARD 同速、50% 约束被破坏
#   - NFR-01 50% 约束验证（边界 HARD_MIN vs EASY_MIN×0.5）：
#       HARD_MIN=50 ≤ EASY_MIN×0.5=50 ✅ 边界成立；
#       任意 score 下 HARD ≤ EASY×0.5 恒成立（公式单调 + 各档下限比 ≤0.5）；
#       区间内（HARD 未达下限前）：HARD=100-3s ≤ EASY=250-4s ⇒ 100-3s ≤ 125-2s ⇒ s≥-25 恒成立 ✓
def speed_curve(score: int, difficulty: Difficulty) -> int:
    ...
    约束（NFR-01）：
      ...
      - tick_ms 不低于该档位独立下限 MIN_TICK_MS[difficulty]（EASY=100 / MEDIUM=80 / HARD=50）
    """
...
# 三档独立下限（per-difficulty dict；满足 NFR-01 量化：HARD ≤ EASY×0.5）
#   - EASY_MIN=100 / MEDIUM_MIN=80 / HARD_MIN=50
#   - 不再使用全局单一 MIN_TICK_MS=50（迭代 2 r1 P1-1 修订）
...
| INV-5 | `pending_direction` 若非 None **且蛇长 ≥ 2**，则与 `direction` 非 opposite（受理时校验）；**长度 == 1 特例除外**（§2.4/§3.2 set_direction 反向规则：长度 1 允许反向生效） | 反向移动禁止 | 1/2 修订（迭代 1 遗留 P2-A 闭环） |
...
| `MIN_TICK_MS` | `Dict[Difficulty, int]`（per-difficulty 常量） | 三档独立下限：`{EASY:100, MEDIUM:80, HARD:50}`（r2 修订：保证 NFR-01 50% 约束在 score 极大时仍成立） | **2 新增** |
...
└── errors.py            # InvalidStateError（**迭代 2 必须同步从 errors.py 与 __init__.py 删除 DirectionError 定义与 re-export**，避免残留死代码或下游 import 不一致；r2 修订：P3-G 闭环）
```
...
| 公式 | `tick = 250/160/100`（常量） | `tick = max(MIN_TICK_MS[d], base - k*score)`；`base/k` 三档独立，**下限 per-difficulty（100/80/50）** |
...
8. **params/types 模块导入顺序与 property 绑定（r2 修订：P3-D 闭环）**：由于 `Difficulty` 在 `types.py`、`speed_curve` / `MIN_TICK_MS` 在 `params.py`、`Difficulty.base_tick_ms` property 体调用 `speed_curve(0, self)`——若按直觉写 `from .types import Difficulty; from .params import speed_curve; class Difficulty: ... @property def base_tick_ms: return speed_curve(0, self)` 会因 `params` 在 `types` 之前导入时 `Difficulty` 尚未定义而报错。**沿用迭代 1 已落地的解决方案**：`types.py` 内只定义 `Difficulty` 枚举本身（不含 property）；`params.py` 在 import `Difficulty` 后**运行时**赋值 `Difficulty.base_tick_ms = property(...)`；FO 照此模式实现，**禁止**在 `types.py` 内 `from .params import speed_curve`。
9. **回调异常不捕获**（C2-3）：core 不在 `step` 内 try/except 包回调；调用方负责隔离。理由：core 鲁棒性边界 = "状态推进正确"，持久化失败属 app 决策。**pure-function 语义（r2 修订：P2-C 闭环）**：回调抛异常时 step 立即向外抛，**本 tick 的 new_state 不交付、调用方拿不到**；旧 state 也未推进（step 入口处先 copy → 计算 → 回调 → 返回，异常发生在回调，未 return 即放弃）。调用方（app）正确处置：本 tick 不推进、或重开对局；**不得用旧 state 继续 step**（否则下一 tick 重复吃同一食物、反复触发同一失败回调）。
10. **`_score_callback` 字段声明（r2 修订：P3-E 闭环）**：作为 frozen dataclass 字段，必须 `field(repr=False, compare=False)` —— 避免 repr 打印回调引用导致日志噪声、避免 eq 比较回调实例不稳定（`Callable` 无可靠 `__eq__`）。`GameState.__init__` 的关键字参数 allowed 集合需包含 `score_callback`（沿用迭代 1 自定义 `__init__` 结构），`_build_initial` 返回 dict 需补 `_score_callback` 字段；遗漏则 `dataclasses.replace` 时 AttributeError。
...
- **「默认节拍」口径说明（r2 修订：P3-F 闭环）**：规格 NFR-01「默认节拍 ≤ 200ms」与 FR-05「简单档 250ms」措辞存在歧义（若「默认」=简单档则 250 > 200）。**本设计与架构口径一致**：「默认节拍」= 普通档（MEDIUM）= `speed_curve(0, MEDIUM) = 160ms ≤ 200ms ✅`；简单档 250ms 是另一独立档位（用于"慢速适应"场景），不影响 NFR-01 验收。三档节拍从慢到快：250 / 160 / 100，默认推荐普通档。
...
| **`speed_curve` score 极大** | **钳制 `tick_ms >= MIN_TICK_MS[difficulty]`，不抛错**（如 MEDIUM score=100 → max(80, 160-4*100)=max(80,-240)=80；EASY score=100 → 100；HARD score=100 → 50） | **2 新增（r2 修订：per-difficulty 下限）** |
| **`speed_curve` score 为负** | **直接代入公式**（如 EASY score=-1 → max(100, 250-4*-1)=max(100,254)=254；不受下限影响因为原始值已 > 下限）；**UT 不承诺负 score 业务语义，但函数不抛错** | **2 新增（r2 修订）** |
...
| **得分回调抛异常** | **异常向外传；本 tick 的状态更新随异常一起丢失（pure-function 语义：调用方拿不到 new_state，旧 state 也未推进）**；调用方（app）的正确处置是「本 tick 不推进」或「重开对局」，**不得用旧 state 继续 step**（否则下一 tick 重复吃同一食物、反复触发同一失败回调） | **2 新增（r2 修订：P2-C 闭环）** |
...
class _GameCoreBase(unittest.TestCase):
    ...
    def make_len1_state(self, rng=None, difficulty=None):
        """长度 1 蛇身状态（特例 UT 专用）。
        走 dataclasses.replace 直接替换（迭代 1 test_state_reversal_block.py 先例，已验可用），
        不走公开构造（GameState 构造默认长度 3，无公开构造路径）。
        r2 修订：迭代 1 遗留 P2-B 闭环。"""
        import dataclasses
        from game_core.types import Snake, Point
        base = self.make_small_state(rng=rng, difficulty=difficulty)
        return dataclasses.replace(
            base,
            snake=Snake((Point(base.width // 2, base.height // 2),)),
            pending_direction=None,
        )
```
...
| 10 | 幂等 | set_direction(同方向) 后 `after.snapshot() == before.snapshot()`（GameState 不可比，按快照比；r2 修订：迭代 1 遗留 P3-C 闭环） |
...
| 25 | **speed_curve 三档独立下限** | `speed_curve(100, MEDIUM) == MIN_TICK_MS[MEDIUM] == 80`；`speed_curve(100, EASY) == MIN_TICK_MS[EASY] == 100`；`speed_curve(100, HARD) == MIN_TICK_MS[HARD] == 50`（r2 修订：每档下限独立） |
...
| 39 | **on_score 回调异常不捕获 + pure-function 语义** | `state = GameState(..., score_callback=lambda s: raise RuntimeError("test"))`；`state.step()` 抛 `RuntimeError`；**断言旧 state 未被污染**（`before == state.snapshot()`）—— new_state 随异常丢失、不可达，符合 step 纯函数语义（r2 修订：P2-C 闭环） |
...
1. 写 `test_speed_curve.py` → 跑（红）→ 写 `params.speed_curve` + `MIN_TICK_MS: Dict[Difficulty, int]`（绿，覆盖 NFR-01 量化 + 三档独立下限；r2 修订：dict 而非 int）
...
## 附录 C：迭代 2 r1 → 迭代 2 r2 修订对照表（回应 SE 评审 FAIL）

> 本表逐条对应 SE 评审 `snake-linux-game-core-design-iter2-r1.md` 的 P1~P3。

| SE 评审意见 | 级别 | 本文档修订位置 | 关键变化 |
|------------|------|----------------|----------|
| **P1-1** 加速曲线数学不满足 NFR-01 50% 约束 | 阻塞 | §1.2 公式 + §1.2 `MIN_TICK_MS` 类型 + §2.7 MIN_TICK_MS 行 + §3.6 公式行 + §4.1 默认节拍口径 + §4.5 score 极大/负数行 + §5.4 #25 + 附录 C | **三档独立下限** `MIN_TICK_MS: Dict[Difficulty, int] = {EASY:100, MEDIUM:80, HARD:50}`；公式 `max(MIN_TICK_MS[d], base - k*score)`；验证 HARD_MIN=50 ≤ EASY_MIN×0.5=50 ✅ 边界成立；任意 score 满足 `HARD ≤ EASY×0.5` 恒成立（公式单调 + 下限比 ≤0.5） |
| **P2-A** INV-5 与长度 1 反向特例仍矛盾 | 应修订 | §1.4 INV-5 | 改为"若非 None **且蛇长 ≥ 2** 则非 opposite；长度 1 特例除外"，注明 §2.4/§3.2 set_direction 反向规则 |
| **P2-B** 长度 1 状态无公开构造途径 | 应修订 | §5.2 夹具 | 新增 `make_len1_state()` 方法，注明走 `dataclasses.replace` 替换 Snake（迭代 1 `test_state_reversal_block.py` 先例） |
| **P2-C** 回调异常时"状态字段已正确更新"不可观察 | 应修订 | §2.4 数据传递方式（已部分覆盖）+ §3.7 #9 实现注意（pure-function 语义详述）+ §4.6 错误矩阵行 + §5.4 #39 断言改写 | 明示"本 tick new_state 随异常一起丢失、不可达；调用方拿不到，旧 state 也未推进"；UT #39 改断言 `before == state.snapshot()`（旧 state 未污染） |
| **P3-A** 状态机图 NEW 未定义 | 建议 | §3.5 状态机图 | `NEW ──▶ OVER` 改为 `RUN(初始) ──▶ OVER` |
| **P3-C** UT #10 比较类型错误 | 建议 | §5.4 #10 | 改为"`after.snapshot() == before.snapshot()`"（按快照比；GameState 字面不可比） |
| **P3-D** §2.1 示例与 §3.1 文件组织冲突（types→params 导入时序） | 建议 | §3.7 #8 实现注意 | 注明"params 在 types 之后导入 + 运行时 property 绑定"模式（沿用迭代 1 已落地）；禁止在 `types.py` 内 `from .params import speed_curve` |
| **P3-E** `_score_callback` frozen 字段实现注意未说明 | 建议 | §3.7 #10 实现注意 | 注明必须 `field(repr=False, compare=False)`；`__init__` 关键字参数 + `_build_initial` 字段补全要求 |
| **P3-F** NFR-01 默认节拍 ≤ 200ms 与简单档 250ms 措辞歧义 | 建议 | §4.1 可维护性首段 | 注明"默认节拍"= 普通档（MEDIUM）= 160ms ≤ 200ms ✅；简单档 250ms 是独立档位 |
| **P3-G** `DirectionError` 删除属破坏性变更需明确迁移 | 建议 | §3.1 文件组织 `errors.py` 行 | 注明"迭代 2 必须同步从 `errors.py` 与 `__init__.py` 删除 DirectionError 定义与 re-export"，避免残留死代码或下游 import 不一致 |

**未变更**（r2 与 r1 一致部分）：
- 数据结构主体（Point/Direction/Difficulty/GameStatus/Snake/Food/GameState/Snapshot 字段与不变量 INV-1/2/3/4/6/7 不变）；
- 数据传递方式通道表；
- §2 对外接口（除 `MIN_TICK_MS` 类型从 `int` 改为 `Dict[Difficulty, int]` 外）；
- §3.2 核心流程伪代码（仍有效；速度计算走 per-difficulty 下限）；
- §3.3 食物生成、§3.4 初始布局、§3.5 状态机（除 NEW→RUN(初始) 一处修订）；
- DFx / 可测试性 / 资源评估主体；
- UT 框架 §5.1 测试组织、§5.3 断言规范、§5.5 覆盖率目标、§5.6 UT 运行命令、§5.7 TDD 步骤（除步骤 1 MIN_TICK_MS dict 类型调整外）；
- 附录 A 接口预告；
- 迭代 1 评审 PASS 的所有架构符合性 14 项。
...
| C2-8 | 约束 | §1.2 / §4.5 / §5.4 #22~25 | NFR-01 量化约束：score=0 时三档 250/160/100；任意 score `HARD ≤ EASY*0.5`；下限 **per-difficulty（100/80/50，r2 修订）**；speed_curve 单调不增 |
